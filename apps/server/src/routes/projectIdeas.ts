 ...
 */
export function projectIdeaRoutes(db: Db): Router {
  const router = Router();
  ...
  router.get(
    ...
    handler(async (req, res) => {
      ...
      const items = await allProjectIdeas(db, query.status);
  ...
  router.post(
    ...
    handler(async (req, res) => {
      ...
      if (body === null) return;
      res.status(201).json(await createProjectIdea(db, actorOf(req), body));
    }),
  );
  ...
  router.patch(
    ...
    handler(async (req, res) => {
      ...
      if (body === null) return;
      res.json(await updateProjectIdea(db, actorOf(req), param(req, 'humanId'), body));
    }),
  );
  ...
  router.delete(
    ...
    canWrite,
    handler(async (req, res) => {
      await softDelete(db, actorOf(req), 'project_idea', param(req, 'humanId'));
      res.status(204).end();
    }),
  );
