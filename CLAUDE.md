...
- **At most 12 MCP tools**, and total tool-definition size stays inside its token budget — both asserted by contract tests. Long content goes through **resources** (`foreman://<code>/architecture#deployment`), never tool output. ADR-002 caps the *verb* surface, not the kinds one verb reaches: `foreman_create` makes projects, requirements, tasks and phases. ADRs, risks, decisions and terms are deliberately absent from it — each carries a body `foreman_update` cannot write, so creating one over MCP would make a titled shell nothing could fill. Those go through the API with a write-scoped token.
