...
Foreman tracks itself as project **`FRM`** — 158 requirements, 11 phases, 14 ADRs. It has since the
...
- **`foreman_delete` deletes ideas, and nothing else** (ADR-014, `proposed`). ADR-002 made deletion absent from the shim rather than gated, because what a delete costs is its *citations* and a tool call cannot see them. An idea is cited by nothing, so the absence is narrowed rather than reversed: `DeleteInput` refuses any other prefix, in `packages/shared`, and names the console instead.
